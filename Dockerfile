...
# Install all packages from the hash-verified lock file.
...
#
# Note: torch is installed from PyPI (CUDA-capable wheel on linux/amd64).
# A prior pattern pre-installed torch from the PyTorch CPU index first, but
# that wheel uses a +cpu local version tag whose hash is absent from the lock
