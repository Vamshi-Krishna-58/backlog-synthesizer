...
# CPU-only PyTorch installed first: sentence-transformers depends on torch, and
# the default index serves a ~3 GB CUDA wheel. Forcing CPU wheel first means pip
# sees torch already satisfied when processing requirements-lock.txt — drops ~2 GB.
# requirements-lock.txt is generated by:
#   pip-compile --generate-hashes --allow-unsafe --output-file requirements-lock.txt requirements.txt
# Regenerate it when requirements.txt changes.
RUN pip install --upgrade pip \
 && pip install --prefix=/install --index-url https://download.pytorch.org/whl/cpu torch \
