...
No real API calls. Tests cover:
  ...
  4. `ParserAgent.run` forwards vision attachments through to the LLM
...
def test_parser_agent_forwards_vision_attachments():
    from agents.parser_agent import ParserAgent
    from memory.store import MemoryStore
    from memory.audit_log import AuditLog

    spy = _SpyClaude()
    parser = ParserAgent(tool=spy, memory=MemoryStore(), audit=AuditLog())
...
def test_parser_agent_no_images_means_no_kwarg(monkeypatch):
    ...
    parser = ParserAgent(tool=strict, memory=MemoryStore(), audit=AuditLog())
    parser.run("transcript", vision_attachments=None)
    assert len(strict.calls) == 1


def test_parser_agent_with_only_images_uses_placeholder_transcript():
